                ...
                <div className="min-h-screen bg-dark-5">
                    <Container className="py-8 bg-white">
                        <div className="flex justify-between items-center mb-4">
                            <button type="button" className="w-8 h-8 flex items-center justify-center bg-dark-5 rounded-full" onClick={() => navigate(-1)}>
                                <FiArrowLeft />
                            </button>
                                ...
                                <div className="flex flex-row items-center justify-center gap-4">
                                    {getItem.tags.length > 0 ?
                                        getItem.tags.map((tag) =>
                                            <span className="bg-primary-10 text-primary-100 rounded-lg px-4 py-2" key={tag.id}>{tag.nome}</span>
                                        )
                                        : null}
                                </div>
                            </div>
...
function CardAlternativa({ correct, item }) {
    return (
        <div className={`flex bg-white mb-4 px-4 py-2 rounded-lg gap-4 ${correct ? "outline outline-4 outline-primary-60" : ""}`}>
            {correct ?
