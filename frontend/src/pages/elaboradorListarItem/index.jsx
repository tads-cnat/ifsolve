...
export default function ElaboradorListarItem() {
    const { getListItens, setListItens, setItens, setCurrentPage } =
        useContext(GlobalContext);

    useEffect(() => {
        setCurrentPage("questoes");
        GetItemsPU(setItens, setListItens).catch(() => {
            toast.error("Opss...Erro ao listar itens.");
        });
    }, []);
        ...
        <SidebarLayout>
            <div className="my-5">
                <div className="flex justify-between">
                    ...
                    <Link
                        to="/criar/item"
                        className="bg-primary-100 flex px-4 py-2 items-center gap-2 rounded-lg"
                    >
                        <FiPlus /> Questão
                    </Link>
                </div>
                ...
                <div className="flex justify-between mb-5">
                    <span className="basis-6/12">
                        Encontramos <b> {getListItens.length} questões! </b>🙌
                    </span>
